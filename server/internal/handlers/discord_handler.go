...
func (h *DiscordHandler) HandleGetGuildRoles(c *fiber.Ctx) error {
	guildID := c.Params("id")
	if guildID == "" {
		return utils.JSONError(c, fiber.StatusBadRequest, "guildId parameter is required in the path route")
	}
...
func (h *DiscordHandler) HandleGetCategories(c *fiber.Ctx) error {
	...
	if err != nil {
		...
		return utils.JSONError(c, fiber.StatusInternalServerError, "Failed to retrieve Discord categories: "+err.Error())
	}
...
func (h *DiscordHandler) HandleCreateCategory(c *fiber.Ctx) error {
	...
	if req.Name == "" {
		return utils.JSONError(c, fiber.StatusBadRequest, "Category name cannot be empty")
	}
	...
	if err != nil {
		...
		return utils.JSONError(c, fiber.StatusInternalServerError, "Failed to create category on Discord: "+err.Error())
	}

	return utils.JSONSuccess(c, fiber.StatusCreated, category)
}
